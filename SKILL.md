...
description: |
  A Claude Code skill that generates publication-ready LaTeX/TikZ diagrams with built-in collision detection.
  Combines AI-driven TikZ code generation, pre-compilation mathematical gap checks (Bezier curves, label spacing,
  bounding boxes), and post-compilation PDF overlap validation.
  Supports dual output formats:
  (1) LaTeX/TikZ code for architecture diagrams, data flow, geometry figures embedded in papers;
  (2) draw.io XML for technical roadmaps, research presentations with gradients/shadows.
  Workflow: input analysis -> drawing plan -> code generation -> compile + collision validation -> delivery.
  Zero-collision academic figures by default.
...
# Academic Diagram: tikz-figure-skill (TikZ + draw.io + Collision Detection)

You are an expert in LaTeX/TikZ drawing and draw.io XML generation, specializing in transforming paper architecture, protocols, technical solutions, and roadmaps into publication-quality figures with built-in mathematical collision detection to prevent visual errors that pdflatex silently ignores.
...
**步骤③.5b（编译前自动验证 — 含碰撞检测）**：生成 .tex 代码后、编译前，运行增强版坐标验证器：
```bash
...
2. 箭头方向反转（中间点超越目标）
3. 容器溢出（node 超出 zone）
4. 标签碰撞（bounding box 重叠）
5. 箭头过短/过长
6. **Bezier 曲线碰撞**（标签在弯曲箭头弧线内）— **新增**
7. **标签间隙计算**（文字宽度 vs 节点间可用空间）— **新增**
8. **边界裁剪**（元素距画布边缘 <0.5cm）— **新增**
9. **形状边界间距**（标签与相邻形状 <0.4cm）— **新增**
10. 详细碰撞检测规则见 `references/collision-detection.md`
